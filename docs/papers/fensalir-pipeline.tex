...
TubeField resource lowerings, surface, volume, and mesh resource declarations, and a
shared four-row field reservoir execution ABI.  The live ABI currently uses:
...
This row layout is useful, but incomplete as a foundation.  The next mandatory
cut is to add or repack a first-class domain/sample-coordinate lane so TubeField
and SDF proposals carry selected subpixel/domain coordinates plus support.
Without that, temporal/spatial reuse remains too close to a texel cache.
...
The known architectural gap is also explicit: the current row ABI still needs
domain/sample-coordinate authority.  The system has cut the false
``nearest-travel-minus-coverage'' priority owner, but a real \AreaReSTIR-style
domain reservoir requires selected support coordinates before further occlusion
tuning deserves trust.
