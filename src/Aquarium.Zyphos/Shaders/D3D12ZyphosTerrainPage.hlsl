...
GameCultGeometryPlanetaryPageSample zyPageErosion(float3 direction, float radius, float spacing, float parent_spacing)
{
    float3 dir=normalize(direction);
    float field=zySphericalField(dir);
    GameCultGeometryPlanetarySurfaceSample child=zyAdvancedErosionSurface(dir,field,radius,spacing);
    GameCultGeometryPlanetarySurfaceSample parent=(GameCultGeometryPlanetarySurfaceSample)0;
    bool has_parent=parent_spacing>0.0;
    if(has_parent)parent=zyAdvancedErosionSurface(dir,field,radius,parent_spacing);
    return gamecult_geometry_planetary_residual_sample(child,parent,has_parent);
}
...
void D3D12ZyphosTerrainPageCS(uint3 id : SV_DispatchThreadID)
{
    ...
    GameCultGeometryPlanetaryPageSample sample=zyPageErosion(dir,radius,spacing,parent_spacing);
    GameCultGeometryPlanetarySurfaceSample child=zyAdvancedErosionSurface(dir,zySphericalField(dir),radius,spacing);
