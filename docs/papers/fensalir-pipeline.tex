...
physical rolling column, curve coordinate, tube support, and explicit motion
when available.  SDF proposals reconstruct object-local hit coordinates and
mark object-motion replay.  Temporal and spatial reuse now reject missing
domain state and include support overlap in the validation weight.
...
domain/sample-coordinate authority, but temporal reuse still needs full
producer re-evaluation after support overlap rather than treating overlap as
the whole legal shift story.  The system has cut the false
``nearest-travel-minus-coverage'' priority owner and the side-cache temptation;
the next proof cut is producer-domain replay with measured rejection precision.
