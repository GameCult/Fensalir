...
void D3D12PlanetaryTopologyParityCS(uint3 id : SV_DispatchThreadID)
{
    ...
    if (input.coordinate_mode.z < 0.5)
    {
        ...
        output.direction_face = float4(gamecult_geometry_planetary_face_direction(face, input.coordinate_mode.xy), (float)face);
        output.coordinate_valid = float4(input.coordinate_mode.xy, 0.0, 1.0);
    }
    else if (input.coordinate_mode.z < 1.5)
    {
        int face;
        float2 coordinate = gamecult_geometry_planetary_face_coordinate(input.direction_face.xyz, face);
    ...
    else if (input.coordinate_mode.z < 2.5)
    {
        ...
        output.coordinate_valid = float4(gamecult_geometry_planetary_equirectangular_forward(input.direction_face.xyz), 0.0, 1.0);
    }
    else if (input.coordinate_mode.z < 3.5)
    {
        output.direction_face = float4(gamecult_geometry_planetary_equirectangular_inverse(input.coordinate_mode.xy), 0.0);
        output.coordinate_valid = float4(input.coordinate_mode.xy, 0.0, 1.0);
    }
    else if (input.coordinate_mode.z < 4.5)
    {
        output.direction_face = float4(normalize(input.direction_face.xyz), 0.0);
        output.coordinate_valid = float4(gamecult_geometry_planetary_equal_earth_forward(input.direction_face.xyz), 0.0, 1.0);
    }
    else if (input.coordinate_mode.z < 5.5)
    {
        output.direction_face = float4(gamecult_geometry_planetary_equal_earth_inverse(input.coordinate_mode.xy), 0.0);
        output.coordinate_valid = float4(input.coordinate_mode.xy, 0.0, 1.0);
    }
    else if (input.coordinate_mode.z < 6.5)
    {
        float2 coordinate;
        bool valid=gamecult_geometry_planetary_projection_forward(input.direction_face.xyz,(int)input.coordinate_mode.w,input.projection_parameters.xyz,coordinate);
    ...
    {
        float3 direction;
        bool valid=gamecult_geometry_planetary_projection_inverse(input.coordinate_mode.xy,(int)input.coordinate_mode.w,input.projection_parameters.xyz,direction);
