...
also re-evaluates the shifted selected logical column/curve coordinate against
the live TubeField buffer when exactly one replay batch is bound.  Multi-batch
TubeField replay remains invalid until a producer-keyed replay manifest maps
selected samples to the correct source buffer and constants.  SDF object-hit
...
domain/sample-coordinate authority, and the first TubeField replay path
re-evaluates a selected logical column/curve coordinate in the current frame for
the single-batch proof case.  SDF object-hit replay now carries the previous
selected hit through object motion and validates it against current
ray/travel/support.  The next proof cuts are producer-keyed multi-batch replay
manifests, exact producer SDF replay, and measured rejection precision.
