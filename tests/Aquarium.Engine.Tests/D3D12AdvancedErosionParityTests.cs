...
public sealed class D3D12AdvancedErosionParityTests
{
    ...
    public void AdvancedErosionMatchesCultMathCpuAcrossDeterministicCorpus()
    {
        ...
        var fixedCases = new[]
        {
            new Case(new float2(0.42f, 0.31f), new float3(0.5f, 0.1f, -0.2f), 0.0f, parameters),
            new Case(new float2(1.7f, -3.2f), new float3(-0.2f, 0.0f, 0.0f), -0.7f, parameters with { Normalization = 0.0f, Octaves = 1 }),
            new Case(new float2(50.5f, 77.25f), new float3(0.8f, -0.3f, 0.4f), 0.9f, parameters with { Normalization = 1.0f, Octaves = 8 }),
            new Case(new float2(-8.125f, 3.75f), float3.zero, 0.0f, parameters with { Rounding = float4.zero }),
        };
        ...
        var cases = fixedCases.Concat(Enumerable.Range(0, 2048).Select(index =>
        {
            ...
            return new Case(
                ...
                Next(-1, 1),
                parameters with { Octaves = 1 + index % 8, Normalization = index % 3 * 0.5f });
        })).ToArray();
        ...
        {
            var cpu = AdvancedErosionFilter.Sample(cases[index].Position, cases[index].Base, cases[index].Fade, cases[index].Parameters);
    ...
    private readonly record struct Case(float2 Position, float3 Base, float Fade, AdvancedErosionParameters Parameters);

    [StructLayout(LayoutKind.Sequential)]
    private readonly record struct ParityInput(Vector4 PositionBase, Vector4 SlopeFade, Vector4 Scalar0, Vector4 Rounding, Vector4 Onset, Vector4 Scalar1, Vector4 Scalar2)
    {
