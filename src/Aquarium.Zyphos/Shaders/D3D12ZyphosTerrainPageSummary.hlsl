...
void D3D12ZyphosTerrainPageSummaryCS(uint3 id : SV_DispatchThreadID)
{
    ...
    float4 summary=gamecult_geometry_planetary_summary_empty();
    [loop] for(uint index=0;index<page_sample_count;index++)
    {
        PageOutput sample=summary_inputs[page_sample_offset+index];
        summary=gamecult_geometry_planetary_summary_accumulate(summary,sample.height_gradient,sample.masks.w);
    }
    summary_outputs[page_summary_index].bounds=summary;
    summary_outputs[page_summary_index].metadata=float4(page_sample_count,page_sample_offset,page_summary_index,1);
}
