...
float zyPageErosion(float3 direction, float radius, float spacing)
{
...
void D3D12ZyphosTerrainPageCS(uint3 id : SV_DispatchThreadID)
{
    ...
    PageInput input=page_inputs[id.x]; float3 dir=normalize(input.direction_radius.xyz); float radius=input.direction_radius.w; float spacing=input.sampling.x;
    ...
    float angular=max(spacing/radius,1.0e-6); float center=zyPageErosion(dir,radius,spacing);
    float dx=(zyPageErosion(normalize(dir+tangent_x*angular),radius,spacing)-zyPageErosion(normalize(dir-tangent_x*angular),radius,spacing))/(2.0*spacing);
    float dy=(zyPageErosion(normalize(dir+tangent_y*angular),radius,spacing)-zyPageErosion(normalize(dir-tangent_y*angular),radius,spacing))/(2.0*spacing);
